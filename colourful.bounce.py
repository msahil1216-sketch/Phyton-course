...
# Sprite class reprsenting the moving object
class Sprite(pygame.sprite.Sprite):
    ...
    def __init__(self, color, height, width):
        ...
        super().__init__()
        # Craete the sprite surface with dimensions and color
        self.imgae = pygame.Surface(width, height) 
        self.image.fill(color)
        # Get h sprite's rect defining its position and size
        self.rect = self.image.get_rect()
        # Set intial velocity with random direction
...
# Function to changen the background color
...
# Set the window little
pygame.disaplay.set_caption("Boundary Spritre")
...
while not exit:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            exit = True
        # If the sprite color chnage event is a triggered, change the sprite's color 
